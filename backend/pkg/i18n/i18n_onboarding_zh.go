...
func init() {
	RegisterBundle(LangZhCN, map[string]string{
		...
		"onboard.daemon.hint_unix":         "提示: 修复错误后重新运行 `openacosmi gateway install`。",
		...
		"onboard.completion.hint":   "Shell 补全: 运行 `openacosmi completion %s >> %s`\n然后重启 shell 或运行: source %s",
		...
		"onboard.finalize.run_manual": "运行: openacosmi",
